...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <form action="{{ route('admin.produk-layanan.update', $produkLayanan) }}" method="POST"
                ...
                <div class="p-6 space-y-6">
                    ...
                    <!-- Materi -->
                    <div x-data="{ items: {{ json_encode(old('materi', $produkLayanan->materi ?? [])) }} }">
                    ...
                    <!-- Manfaat -->
                    <div x-data="{ items: {{ json_encode(old('manfaat', $produkLayanan->manfaat ?? [])) }} }">
                    ...
                    <!-- Persyaratan -->
                    <div x-data="{ items: {{ json_encode(old('persyaratan', $produkLayanan->persyaratan ?? [])) }} }">
                    ...
                    <!-- Alur Proses -->
                    <div x-data="{ items: {{ json_encode(old('alur_proses', $produkLayanan->alur_proses ?? [])) }} }">
                    ...
                    <!-- Jalur Pelatihan -->
                    <div x-data="{ items: {{ json_encode(old('jalur_pelatihan', $produkLayanan->jalur_pelatihan ?? [])) }} }">
                    ...
                    <!-- Banner Image -->
                    <div>
                        <label for="gambar_banner" class="block text-sm font-medium text-gray-700">
                            {{ $produkLayanan->gambar_banner ? 'Ganti Banner' : 'Banner' }}
                        </label>
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            <option value="aktif"
                                {{ old('status', $produkLayanan->status) == 'aktif' ? 'selected' : '' }}>
                                Aktif
                            </option>
                            <option value="nonaktif"
                                {{ old('status', $produkLayanan->status) == 'nonaktif' ? 'selected' : '' }}>
                                Non Aktif
                            </option>
                        </select>
                        @error('status')
